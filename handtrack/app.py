...
def main(camera_index=None):
    """
    Open a camera feed and draw hand landmarks only.
    Press 'q' or ESC to quit.
    """
    cap = cv2.VideoCapture(0 if camera_index is None else camera_index)
    if not cap.isOpened():
        raise RuntimeError("Could not open camera {}".format(camera_index))
    ...
    print("Starting hand tracking. Press 'q' or ESC to quit.")
    frame_count = 0
    while True:
        ok, frame = cap.read()
        if not ok:
